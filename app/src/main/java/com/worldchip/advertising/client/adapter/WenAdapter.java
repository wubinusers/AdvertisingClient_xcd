...
public class WenAdapter extends BaseAdapter {
    ...
    public final static int RECREATE = 0x423;
    ...
    public WenAdapter(Context context, Handler handler) {
        this.mHandler = handler;
		mContext = context;
        mInflate = LayoutInflater.from(context);
        mShare = context.getSharedPreferences("wen", 0);
        // 淇濆瓨缁勬暟銆�
    ...
    public void addItem(int position) {
        if (groups < 5) {
            groups++;
            mShare.edit().putInt("groups", groups).commit();
            mShare.edit().putString(autoNextGroup(groups), "3009:0021:00").commit();
            groupDetails.add(position + 1, "3009:0021:00");
            mHandler.sendEmptyMessage(RECREATE);
        }
    }
    ...
    public void resetAllChecked() {
        ...
        for (int i = 0; i <= groups; i++) {
            str = mShare.getString("group" + i, "3009:0021:00");
    ...
    public void setChecked(int position) {
        if (getChecked(position)) {
            mShare.edit().putString("group" + position, "30" + getText(position, 2, 12)).commit();
        } else {
            mShare.edit().putString("group" + position, "31" + getText(position, 2, 12)).commit();
        }
    }


    public boolean getChecked(int position) {
        String str = mShare.getString("group" + position, "3009:0021:00").substring(1, 2);
        if (str.equals("0")) {
            return false;
        }
    ...
    private void saveSharePre(int position) {
        ...
        mShare.edit().putString("group" + position, 3 + getCheckedString(position) + onShareHour + ":" + onShareMin + offShareHour + ":" + offShareMin).commit();
    }

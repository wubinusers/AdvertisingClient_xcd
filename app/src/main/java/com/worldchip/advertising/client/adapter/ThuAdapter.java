...
public class ThuAdapter extends BaseAdapter {
    ...
    public final static int RECREATE = 0x523;
    ...
    public ThuAdapter(Context context, Handler handler) {
        this.mHandler = handler;
		mContext = context;
        mInflate = LayoutInflater.from(context);
        mShare = context.getSharedPreferences("thu", 0);
        // 淇濆瓨缁勬暟銆�
    ...
    public void addItem(int position) {
        if (groups < 5) {
            groups++;
            mShare.edit().putInt("groups", groups).commit();
            mShare.edit().putString(autoNextGroup(groups), "4009:0021:00").commit();
            groupDetails.add(position + 1, "4009:0021:00");
            mHandler.sendEmptyMessage(RECREATE);
        }
    }
    ...
    public void resetAllChecked() {
        ...
        for (int i = 0; i <= groups; i++) {
            str = mShare.getString("group" + i, "4009:0021:00");
    ...
    public void setChecked(int position) {
        if (getChecked(position)) {
            mShare.edit().putString("group" + position, "40" + getText(position, 2, 12)).commit();
        } else {
            mShare.edit().putString("group" + position, "41" + getText(position, 2, 12)).commit();
        }
    }

    public boolean getChecked(int position) {
        String str = mShare.getString("group" + position, "4009:0021:00").substring(1, 2);
        if (str.equals("0")) {
            return false;
        }
    ...
    private void saveSharePre(int position) {
        ...
        mShare.edit().putString("group" + position, 4 + getCheckedString(position) + onShareHour + ":" + onShareMin + offShareHour + ":" + offShareMin).commit();
    }
}
